...
protected:
  ...
  void Serialize(Writer& writer) const {
    // This base class just write out name-value pairs,
    // without wrapping within an object.
    writer.String("name");
    writer.String(name_.c_str(), (SizeType)name_.length());
        // Suppling length of string is faster.
...
public:
  ...
  Dependent(const Dependent& rhs): Person(rhs) {
...
int main() {
  ...
  employees.back().AddDependent(
      Dependent("Lua YIP", 3, new Education("Happy Kindergarten", 3.5))
  );
